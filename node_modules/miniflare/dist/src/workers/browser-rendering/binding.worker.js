...
var BrowserSession = class extends DurableObject {
  ...
  async fetch(_request) {
    ...
    ws.accept(), ws.addEventListener("message", (m) => {
    ...
    let forwardClose = (ws2, e) => {
      e.code === 1005 || e.code === 1006 ? ws2.close() : ws2.close(e.code, e.reason);
    };
    return server.addEventListener("close", (e) => {
      forwardClose(ws, e), this.ws = void 0;
    }), ws.addEventListener("close", (e) => {
      forwardClose(server, e), this.server = void 0;
  ...
  async #checkStatus() {
    if (this.sessionInfo) {
      ...
      url.searchParams.set("sessionId", this.sessionInfo.sessionId), (await this.env[CoreBindings.SERVICE_LOOPBACK].fetch(url)).ok || this.closeWebSockets();
    }
  }
  closeWebSockets() {
