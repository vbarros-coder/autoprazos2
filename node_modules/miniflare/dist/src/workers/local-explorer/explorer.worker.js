...
function getLocalD1Databases(env) {
  ...
  return Object.entries(d1BindingMap).map(([id, bindingName]) => {
    let parts = bindingName.split(":"), workerName = parts.length >= 3 ? parts[2] : "unknown";
    return {
      name: parts.pop() || bindingName,
      uuid: id,
      version: "production",
      workerName
    };
...
function getLocalKVNamespaces(env) {
  ...
  return Object.entries(kvBindingMap).map(([id, bindingName]) => {
    let parts = bindingName.split(":"), workerName = parts.length >= 3 ? parts[2] : "unknown", title = parts.pop() || bindingName;
    return {
      id,
      title,
      workerName
...
function getR2Binding(env, bucket_name) {
  let bindingName = env.LOCAL_EXPLORER_BINDING_MAP.r2[bucket_name];
  return bindingName ? env[bindingName] : null;
}
...
function getLocalR2Buckets(env) {
  let r2BindingMap = env.LOCAL_EXPLORER_BINDING_MAP.r2;
...
app.get("/api/workers", async (c) => {
  let loopback = c.env.MINIFLARE_LOOPBACK, selfWorkerNames = c.env.LOCAL_EXPLORER_WORKER_NAMES, selfSet = new Set(selfWorkerNames);
  try {
    let registry = await (await loopback.fetch("http://localhost/core/dev-registry")).json(), workers = Object.entries(registry).map(
      ([name, def]) => ({
        host: def.host,
