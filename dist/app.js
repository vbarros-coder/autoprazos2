...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Carregar dados iniciais do JSON se disponível
  fetchInitialData();
});
...
async function fetchInitialData() {
  try {
    ...
    if (data && data.length > 0) {
      allData = data.map(d => ({
        ...d,
        data_sinistro: toDate(d.data_sinistro),
        data_entrada: toDate(d.data_entrada),
        data_vistoria: toDate(d.data_vistoria),
        prazo_d2: toDate(d.prazo_preliminar || d.prazo_d2),
        prazo_90d: toDate(d.prazo_90d),
        data_envio_prelim: toDate(d.data_envio_prelim),
        data_ultimo_doc: toDate(d.ultima_recepcao_docs || d.data_ultimo_doc),
        _diasAbertos: d.dias_abertos || d._diasAbertos,
        _urg: d._urg || calcUrgencia(d.situacao, toDate(d.prazo_preliminar || d.prazo_d2), new Date(), toDate(d.data_entrada))
      }));
      
      document.getElementById('data-tag').innerHTML = `<i class="fas fa-check-circle text-green-500"></i> ${allData.length} processos carregados da base.`;
      fillFilters();
      applyF();
    }
  } catch (err) {
